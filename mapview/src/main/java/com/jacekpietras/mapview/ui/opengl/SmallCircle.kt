...
internal class SmallCircle : Circle() {

    override fun draw(mvpMatrix: FloatArray?, cX: Float, cY: Float, radius: Float, color: Int) {
        val data = SmallCircleShapeData(cX, cY, radius, color)

        draw(mvpMatrix, data)
    }

    private class SmallCircleShapeData(cX: Float, cY: Float, radius: Float, colorInt: Int) : OrganizedShapeData(colorInt) {
