...
import com.jacekpietras.zoo.domain.feature.vrp.algorithms.TwoOptHeuristicVRP
...
val vrpModule = module {
    single<StageVRPSolver> {
        StageVRPSolverImpl(
            mapRepository = get(),
            graphAnalyzer = get(),
            vrpAlgorithm = TwoOptHeuristicVRP<Stage>() * (NearestNeighborVRP<Stage>() + TwoOptHeuristicVRP()),
        )
    }
}